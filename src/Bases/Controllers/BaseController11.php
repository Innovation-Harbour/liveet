...
namespace LAGOS_RECYCLE\Controllers;

use DateTime;
use LAGOS_RECYCLE\Domain\Constants;
use LAGOS_RECYCLE\Domain\MailHandler;
...
use LAGOS_RECYCLE\Models\BaseModel;

class BaseController
{

    private const EMAIL_VERIFIED = Constants::EMAIL_VERIFIED;
    private const USER_VERIFIED = Constants::USER_VERIFIED;
    private const DEFAULT_RESET_PASSWORD = Constants::DEFAULT_RESET_PASSWORD;
    ...
    public function sendMail($allInputs)
    {
        $success = '';
        $error = '';
        if (isset($allInputs["email"]) and $allInputs["email"] and isset($options['sendMail']) and $options['sendMail'] == true) {
    ...
    public function getAll(Request $request, ResponseInterface $response, $model): ResponseInterface
    {
        ...
        $payload = array('successMessage' => 'Success', 'statusCode' => 200, 'data' => $data['data']);
    ...
    public function getByColumn(Request $request, ResponseInterface $response, $model, $columnName, $returnColumns = null, $columnValue = null): ResponseInterface
    {
        $json = new JSON();
        ...
        $data = isset($returnColumns) ? $model->getByColumnWithCustomReturnColumns($columnName, $columnValue, $returnColumns) : $model->getByColumn($columnName, $columnValue);
    ...
    public function getByColumnWithRelationships(Request $request, ResponseInterface $response, $model, $columnName, $relationships, $returnColumns = null, $columnValue = null): ResponseInterface
    {
        $json = new JSON();
        ...
        $data = $model->getByColumnWithRelationships($columnName, $columnValue, $relationships);
