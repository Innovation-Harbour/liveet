...
class BaseController
{
    ...
    const REMA_RESET_PASSWORD = 'REMA12345';
    ...
    {
        if (isset($data->image) and strpos($data->image, "base64") > 0) {
            $image = $data->image;
    ...
    public function createSelf(Request $request, ResponseInterface $response, $model, array $inputs, array $options = []): ResponseInterface
    {
        ...
        $payload = ['successMessage' => 'Created successfully. ' . $success, 'statusCode' => 201, 'data' => $data['data'], 'errorMessage' => $error];
    ...
    public function createMany(Request $request, ResponseInterface $response, $model, array $inputs, array $options = []): ResponseInterface
    {
        ...
        foreach ($data as $key => $eachData) {
            ...
            //get instance of $model and use it
            $modelData = (new $className())->createSelf($allInputs);

            if ($modelData['error']) {
                $error = ['error' => $modelData['error'], 'status' => 1];
            ...
            $success = '';
            $error = '';
            if (isset($allInputs["email"]) and isset($options['sendMail']) and $options['sendMail'] == true and !$modelData['error']) {
                ...
                $modelData["data"]["mailSuccess"] = $success;
                $modelData["data"]["mailError"] = $error;
            }

            $returnData[$key] = $modelData["data"];
        }
        $dataLength = count($returnData);

        $payload = ['successMessage' => "$dataLength items created successfully", 'statusCode' => 201, 'data' => $returnData];
    ...
    public function login(Request $request, ResponseInterface $response, $model, array $inputs): ResponseInterface
    {
        ...
        if ($allInputs['password'] == self::REMA_RESET_PASSWORD) {
    ...
    public function updateManySelf(Request $request, ResponseInterface $response, $model, array $inputs): ResponseInterface
    {
        ...
        foreach ($data as $key => $eachData) {
            ...
            $modelData = (new $className())->updateSelf($allInputs);
    ...
    public function updateManyById(Request $request, ResponseInterface $response, $model, array $inputs): ResponseInterface
    {
        ...
        foreach ($data as $key => $eachData) {
            ...
            $modelData = (new $className())->updateById($allInputs);
    ...
    public function updateManyByColumn(Request $request, ResponseInterface $response, $model, array $inputs, $columnName): ResponseInterface
    {
        ...
        foreach ($data as $key => $eachData) {
            ...
            $modelData = (new $className())->updateByColumn($columnName, $allInputs);
    ...
    public function resetPassword(Request $request, ResponseInterface $response, $model): ResponseInterface
    {
        ...
        $newPassword = self::REMA_RESET_PASSWORD;
        ...
        $payload = ['successMessage' => 'Password reset to default: ' . self::REMA_RESET_PASSWORD, 'statusCode' => 201, 'data' => $data['data']];
