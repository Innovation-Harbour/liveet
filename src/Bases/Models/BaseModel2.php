...
namespace LAGOS_WASTE\Models;

use Rashtell\Domain\KeyManager;
use Illuminate\Database\Eloquent\Model;
use LAGOS_WASTE\Domain\Constants;
use LAGOS_WASTE\Domain\PointsManager;
...
class BaseModel extends Model
{
    ...
    public function getSelfTransactions($id)
    {
        ...
        // isset($transactions->property) && $transactions->property;
    ...
    protected function appendDetailsToTransaction($transactions)
    {
        ...
        for ($iterator; $iterator < sizeof($transactions); $iterator++) {
            ...
            $propertyID = $transactions[$iterator]["propertyID"];

            isset($transactions[$iterator]["propertyID"]) && $newTransactions[$iterator]["property"] = PropertyModel::find($propertyID);
        }
    ...
    {
        $model =  ExternalAgentModel::find($id);
    ...
    public function getAgents($id)
    {
        ...
        if (!$agents) {
            return ['data' => [], 'error' => 'No agents yet for this user'];
        }
    ...
    public function getExternalAgents($id)
    {
        ...
        if (!$externalAgents) {
            return ['data' => [], 'error' => 'No external externalAgents yet for this user'];
        }
    ...
    protected function updatePoint($details)
    {
        ...
        if (!$model) {
            return ['error' => 'Does not exist', 'data' => []];
        }
